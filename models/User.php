...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function rules()
    {
        return [
            ...
            [['activated', 'updated_at', 'maintain_point'], 'safe'],
            [['ewallet', 'pinwallet', 'point', 'maintain', 'price'], 'number'],
            [
                [
                    'username',
                    'password_hash',
            ...
            ['email', 'email'],
            [['email'], 'unique'],
            [['avatarFile'], 'file', 'skipOnEmpty' => true, 'extensions' => ['jpg', 'jpeg', 'png', 'gif', 'webp'], 'mimeTypes' => ['image/jpeg', 'image/png', 'image/gif', 'image/webp'], 'maxSize' => 10 * 1024 * 1024, 'on' => 'updateProfile'],
    ...
    public function checkMaintainPoint()
    {
        $timeMaintain = strtotime($this->maintain_point);
        $timeNow = strtotime("now");
        if (!$this->maintain_point) {
            return false;
        } else if ($timeMaintain >= $timeNow) {
            return true;
        } else {
            return false;
        }
